...
class GameDemo {
    ...
    private fun initializeGame() {
        ...
        println("Game initialized with ${gameState.getPlayerCharacters().size} player units and ${gameState.getEnemyCharacters().size} enemy units")
    }
    ...
    private fun printBoard() {
        ...
        for (y in 0 until board.height) {
            ...
            for (x in 0 until board.width) {
                ...
                when {
                    character != null -> {
                        val symbol = when (character.team) {
                            Team.PLAYER -> when (character.characterClass) {
                            ...
                            Team.ENEMY -> when (character.characterClass) {
                                CharacterClass.KNIGHT -> "♚"
                                CharacterClass.ARCHER -> "♜"
                                CharacterClass.MAGE -> "♝" 
                                CharacterClass.HEALER -> "♛"
        ...
        for (unit in playerUnits) {
            ...
            if (targets.isNotEmpty()) {
                ...
                if (possibleMoves.isNotEmpty()) {
                    val nearestEnemy = gameState.getEnemyCharacters()
                        .filter { it.isAlive }
                        .minByOrNull { it.position.distanceTo(unit.position) }
                    
                    if (nearestEnemy != null) {
                        val bestMove = possibleMoves.minByOrNull { 
    ...
    private fun printUnitStatus() {
        ...
        for (unit in gameState.getPlayerCharacters().filter { it.isAlive }) {
            ...
            println("  ${unit.name} (Lv.${unit.level} ${unit.characterClass.displayName}) - HP: ${unit.currentHp}/${stats.hp}, ATK: ${stats.attack}, DEF: ${stats.defense}")
        }
